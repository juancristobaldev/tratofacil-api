...
export class OrderProductResolver {
  ...
  @Query(() => ShippingInfo, {
    name: 'shippingInfoByOrderProduct',
    nullable: true,
  })
  @UseGuards(JwtAuthGuard)
  async getShippingInfoByOrderProduct(
    @Args('orderProductId', { type: () => Int }) orderProductId: number,
  ) {
    return this.orderProductService.findByOrderProductId(orderProductId);
  }
  ...
  async createOrderProduct(
    ...
    @Args('shippingInput') shippingInput: CreateShippingInfoInput,
    @Context() context: any,
  ) {
